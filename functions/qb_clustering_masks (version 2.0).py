...
fornix = load_tractogram('cnm.tck', '100610/avg.nii.gz', bbox_valid_check=False)
...
func_list = []
for file in files:
    os.system('fslroi data.nii.gz b0.nii.gz 0 1')
    os.mkdir(file+'/bundles')
    os.mkdir(file+'/tdi')
    os.mkdir(file+'/masks')
    os.mkdir(file+'/weights')
    #os.system('fslroi '+file+'/data.nii.gz '+file+'/b0.nii.gz 0 1 ')
    func = 'fslmerge -tr '+file+'/bundle_masks.nii.gz '
...
for number in range(cluster_number):
    ...
    for i in range(len(wr_list)):
        ...
        with open(text, 'w') as f:
            f.write('\n'.join(wr_list[i]))
        CMD = 'tckedit '+file+'.tck '+file+'/bundles/cluster'+str(number)+'.tck -tck_weights_in '+text+' -minweight 1.0'
        os.system(CMD)

        CMD = 'tckmap '+file+'/bundles/cluster'+str(number)+'.tck -contrast tdi '+file+'/tdi/t'+str(number)+'.nii.gz -template '+file+'/b0.nii.gz'
        os.system(CMD)

        CMD = 'fslmaths '+file+'/tdi/t'+str(number)+'.nii.gz -bin '+file+'/masks/mask'+str(number)+'.nii.gz'
        os.system(CMD)

        mask = file+'/masks/mask'+ str(number)+'.nii.gz '
        func_list[i] += mask
