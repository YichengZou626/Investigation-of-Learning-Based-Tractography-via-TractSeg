...
#todo: adapt
dataset = "HCP"
DATASET_FOLDER = "/nas/longleaf/home/zyc626/TractSeg/tractseg/HCP"  # source folder
DATASET_FOLDER_PREPROC = "/nas/longleaf/home/zyc626/TractSeg/tractseg/preproc"  # target folder
...
def create_preprocessed_files(subject):
    ...
    exp_utils.make_dir(join(DATASET_FOLDER_PREPROC, subject))

    bb_file_path = join(DATASET_FOLDER, subject, bb_file + ".nii.gz")
    ...
    for idx, filename in enumerate(filenames_data):
        path_src = join(DATASET_FOLDER, subject, filename + ".nii.gz")
        path_target = join(DATASET_FOLDER_PREPROC, subject, filename + ".nii.gz")
