...
def load_training_data(Config, subject):
    ...
    data = load(join(C.DATA_PATH, Config.DATASET_FOLDER, subject, "mrtrix_peaks"))
    seg = load(join(C.DATA_PATH, Config.DATASET_FOLDER, subject, "bundle_masks"))
...
class DataLoaderTraining:
    ...
    def _augment_data(self, batch_generator, type=None):
        ...
        if self.Config.DATA_AUGMENTATION:
            num_processes = 16  # 15 is a bit faster than 8 on cluster
        ...
        batch_gen = MultiThreadedAugmenter(batch_generator, Compose(tfs), num_processes=num_processes,
                                           num_cached_per_queue=2, seeds=None, pin_memory=True)
